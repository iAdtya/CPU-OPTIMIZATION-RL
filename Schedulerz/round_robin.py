...
class RoundRobin(Scheduler):
    ...
    def run(self):
        ...
        while (
            len(deq) != 0 or self.data.size > 0
        ):  # if queue is not empty or data is not empty
            while (
                self.data.size > 0 and self.data[0, 1] == time
            ):  # add all processes to queue that arrive at 'time'
            ...
            if deq:  # if queue is not empty
                ...
                if (
                    consecutive != quantum
                ):  # if quantum is not reached, add to front of queue
            ...
            else:  # if queue is empty, then store -1 in the gantt chart (list)
                self.gantt.append(-1)
